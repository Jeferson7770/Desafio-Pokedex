...
```http
GET /api/relatorios/
GET /api/relatorios/?year=2026&month=6
```
...
1. `GET /api/relatorios/?year=2026&month=6` para carregar o relatorio do mes.
2. Nao tratar resposta zerada como erro — e o comportamento esperado para meses sem dados.
3. `POST /api/relatorios/` para consolidar dados ao fechar o mes (upsert automatico).
